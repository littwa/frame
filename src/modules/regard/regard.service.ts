...
import { AddRegardDto, TextDto, ParamIdDto, TextDtoFind } from 'src/modules/regard/dto/regard.dto';
...
export class RegardService {
  ...
  async createTextAndAddToRegard(file: Express.Multer.File, body: TextDto, regardId: string, req: IRequestExt) {
  ...
  async updateText(file: Express.Multer.File, body: TextDto) {
    ...
    if (img && foundText.public_id) {
      await this.commonService.deleteFromCloudinary(foundText.public_id, EMediaType.Image);
    }
  ...
  async getRegardAggregated(regardId: string) {
    const regard = await this.regardModel.find({ _id: regardId }).populate('list').populate('qualifies');
  ...
  async checkQualifyExec(body, { textId, regardId, qualifyId }, req) {
    const qualify = await this.qualifyModel.findOne({ _id: qualifyId }).exec();
  ...
  async startNextLap(qualifyId: string) {
    ...
    if (!qualify.finishedLap) throw new NotFoundException(`Preceding lap not finished yet`);

    qualify.stat = qualify.stat.map(v => (!v.finishedInQualify ? { ...v, lapChecked: false } : v));
